...
class PortScan_Main(threading.Thread):
    ...
    def run(self):
        try:
            ...
            print("\a  [+] ["+ self.host +"/"+ str(self.port) +" - "+ str(round(pingend, 4)*1000) +u"ミリ秒]: TCP/IP Open | "+ self.serv)
            if (self.senddata.lower() == "none"):
                pass
            elif (self.senddata == "GET / HTTP/1.0\r\n\r\n"):
                sock.send(self.senddata) # Send
                readdata = ""
                while True:
                ...
                line = readdata.find("\r\n\r\n")
                if (line != -1):
                    for show in readdata[0:line].splitlines():
                        print("      "+ show)
                    path = os.path.dirname(os.path.abspath(sys.argv[0]))
                    if (os.path.exists(path +"\\HTML") == False): os.mkdir(path +"\\HTML")
                    with open(path +"\\HTML\\"+ self.host +"-"+ str(self.port) +".html", "w") as f:
                        f.write(readdata[line:len(readdata)])
                    print("")
            else:
                sock.send(self.senddata)              
                print(u"      [受信データ]:"+ sock.recv(1024 * 1024))
        except:
            pass
...
while True:
   ...
   try: # Port Scan Send Data
       ...
       if (senddata == ""):
           ...
           for rd in (("\\n", b"\n"), ("\\r", b"\r"), (b"\\0", b"\0"), (b"\\t", b"\t")):
              senddata = senddata.replace(rd[0], rd[1])
